...
void IWRAM_CODE Refresh_filename(u32 show_offset, u32 file_select, u32 updown, u32 haveThumbnail)
{
	...
	{
		DrawHZText12(pFilename_buffer[offset + xx1 - need_show_folder].filename, char_num1, 1 + 16, showy1, name_color1,
		             1);
		DrawHZText12(pFilename_buffer[offset + xx2 - need_show_folder].filename, char_num2, 1 + 16, showy2, name_color1,
		             1);
...
void Show_ICON_filename_NOR(u32 show_offset, u32 file_select)
{
	...
	for (line = 0; line < need_show; line++)
	{
		...
		DrawPic((u16 *)gImage_icon_nor /*(gImage_icons+2*16*14*2)*/, 0, y_offset + line * 14, 16, 14, 1, gl_color_text,
		        1);
...
u8 Get_saveMODE(u8 Save_num, u32 gamefilesize)
{
	...
	else // Manual selection
	{
		...
		case 0x1:
			...
			break; // SRAM
		case 0x2:
			if (gamefilesize > 0x1200000)
			{                    // some eeprom modify rom
...
void SD_list_L_START(u32 show_offset, u32 file_select, u32 folder_total)
{
	...
	DrawHZText12(pFilename_buffer[show_offset + file_select - folder_total].filename, 20, 60, 75, 0x7fff,
	             1);                                              // file name
			...
			else if (keysdown & KEY_A)
			{
				if (page_num == SD_list)
				{
					...
					if (show_offset + file_select < folder_total)
					{
						...
						strncat(currentpath, pFolder[show_offset + file_select].filename,
						        MAX_path_len - strlen(currentpath) - 1);
