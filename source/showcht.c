...
void Show_KEY_val(u32 total, u32 Select, u32 showoffset)
{
	...
	for (line = 0; line < need_show; line++)
	{
		...
		if (((FM_CHT_LINE *)pCHTbuffer)[showoffset + line].is_section == 1)
		{
			...
			// int res =
			// utf8_check(((FM_CHT_LINE*)pCHTbuffer)[showoffset+line].LINEname,strlen(((FM_CHT_LINE*)pCHTbuffer)[showoffset+line].LINEname));
			// DEBUG_printf(" %x",res);

			// DEBUG_printf(" %x %x %x %x
			// ",((FM_CHT_LINE*)pCHTbuffer)[showoffset+line].LINEname[0],((FM_CHT_LINE*)pCHTbuffer)[showoffset+line].LINEname[1],((FM_CHT_LINE*)pCHTbuffer)[showoffset+line].LINEname[2],((FM_CHT_LINE*)pCHTbuffer)[showoffset+line].LINEname[3]);
...
void Analyze_KEYVAL(FIL *file, u32 total)
{
	...
	for (tol = 0; tol < total; tol++)
	{
		...
		if (((FM_CHT_LINE *)pCHTbuffer)[tol].select == 1)
		{
			...
			buflen = Get_CHT_val(&gfile, ((FM_CHT_LINE *)pCHTbuffer)[current_select].LINEname,
			                     ((FM_CHT_LINE *)pCHTbuffer)[tol].LINEname);
...
u32 Change2cht_folder(u32 chtname)
{
	...
	sprintf(chtnamebuf, "%d%d%d%d", HexToChar(((u8 *)&chtname)[0]), HexToChar(((u8 *)&chtname)[1]),
	        HexToChar(((u8 *)&chtname)[2]), HexToChar(((u8 *)&chtname)[3]));
			...
			for (i = 0; i < filesize / 4; i += 2)
			{
				...
				if (tempbuff[i] == GAMEID)
				{
					...
					if (res != 0)
						return 0;
					memset(chtnamebuf, 0x00, 100);
					sprintf(chtnamebuf, "%d%d%d%d.cht", HexToChar(((u8 *)&chtname)[0]), HexToChar(((u8 *)&chtname)[1]),
					        HexToChar(((u8 *)&chtname)[2]), HexToChar(((u8 *)&chtname)[3]));
					res = f_open(&gfile, chtnamebuf, FA_OPEN_EXISTING);
...
void Open_cht_file(TCHAR *gamefilename, u32 havecht)
{
	...
	else
	{
		...
		u8 *chtmode;
		chtmode = (u8 *)&havecht;
		sprintf(chtnamebuf, "%d%d%d%d.cht", HexToChar(chtmode[0]), HexToChar(chtmode[1]), HexToChar(chtmode[2]),
		        HexToChar(chtmode[3]));
	}
